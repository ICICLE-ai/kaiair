...
(Python stdlib only, no dependencies). The agent launches and supervises the modem
process, translates the persistent control state into `KAIAIR_*` environment at launch,
and exposes the runtime control surface. A separate local web dashboard
(`dashboard/kaiair_dash.py`) drives the agent over SSH and plots the telemetry.

```
        browser ── kaiair_dash.py ── ssh ── kaiair_agent.py ──┬── gnb / nr-uesoftmodem
                                                              ├── /tmp/kaiair_ctrl.json   (control overlay)
                                                              └── /tmp/kaiair_metrics.json (telemetry, 1 Hz)
```
...
file paths, and optionally the peer list for the UDP dev fallback (unused when Xn carries
the exchange). Examples under `codesign-api/`.
...
true-SINR CSI flag only if the node config says the UE is patched. Nothing about the
algorithm is hard-coded in the agent. It only forwards state.

### Runtime control (no restarts)
...
`GET /metrics` returns the latest 1 Hz JSON. The fields most consumers need:

| Field | Meaning |
|---|---|
| `ts`, `rnti` | sample wall time; active C-RNTI (absent when no UE is connected) |
...
link wins over the precomputed mask window, approximately `1/(conflicts+1)`, emergent
rather than configured.

`GET /peers` reports the UDP dev-fallback bus state (not used when Xn carries the
exchange).
...
`/metrics` at about 3 Hz through SSH, keeps a sliding window, and renders: UL PUSCH SINR
with the gamma target line, UL throughput, DL SINR against its own target, UL/DL power,
LDP delivery, and a multi-cell coordination panel (neighbor RSRP, K, ER, conflicts,
active fraction) that appears when peers exist. The control bar drives the endpoints
above: power toggle, UL and DL gamma sliders, LDP deadline, all live.

The host, port, and polling cadence are constants at the top of `kaiair_dash.py`.
